def align_distributions_dynamic_tests(
    ...
    import pandas as pd
    import logging
    from scipy.stats import shapiro, ttest_ind, mannwhitneyu, ks_2samp, chi2_contingency
    ...
    # Fixed: Check debug_flags first, then verbose
    if debug_flags:
    ...
    if seed is None:
        ...
        logger.info(f"Randomly selected seed: {seed}")
    else:
    ...
    def _safe_random_choice(pool):
        """Return random choice from pool, or None if pool is empty"""
        return np.random.choice(pool, 1)[0] if pool else None

    # Balance tuples pre-processing
    if balance_tuples and balance_column:
        for value in balance_tuples:
            ...
            while len(df1_rows) > 1 and len(df2.index) > 0:
                j = np.random.choice(df2.index)
                i = df1_rows.index[0]
                _swap_rows(df2, j, df1, i)
                df1_rows = df1[df1[balance_column].isin(value)]
                df2_rows = df2[df2[balance_column].isin(value)]
            while len(df2_rows) > 1 and len(df1.index) > 0:
        ...
        for metric in metrics:
            ...
            if not scalar_ok:
                ...
                if residual_guidance:
                    ...
                    i = _safe_random_choice(i_pool)
                    j = _safe_random_choice(j_pool)
                            ...
                            if over_mask.any():
                                ...
                                if under_mask.any():
                                    ...
                                    while swaps_done < categorical_swaps_per_iter:
                                        ...
                                        i = _safe_random_choice(src)
                                        j = _safe_random_choice(dst)
        ...
        if gates_ok:
            if has_toster:
                ...
                if all_eq:
                    ...
                    i_pool = _eligible(df1.index)
                    j_pool = _eligible(df2.index)
                    i = _safe_random_choice(i_pool)
                    j = _safe_random_choice(j_pool)
                    if i is not None and j is not None:
                        _swap_rows(df1, i, df2, j)
    ...
    # Reached max_iterations without breaking
